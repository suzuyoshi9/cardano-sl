...
makePubKeyTx ss txInputs txOutputs = runIdentity $
    makeMPubKeyTx (\_ -> Identity ss) (map ((), ) txInputs) txOutputs

makeMOfNTx :: HasConfiguration => Script -> [Maybe SafeSigner] -> TxInputs -> TxOutputs -> TxAux
makeMOfNTx validator sks txInputs txOutputs = runIdentity $
    makeAbstractTx mkWit (map ((), ) txInputs) txOutputs
  where
    mkWit _ sigData = Identity $ ScriptWitness
...
makeRedemptionTx rsk txInputs txOutputs = runIdentity $
    makeAbstractTx mkWit (map ((), ) txInputs) txOutputs
  where rpk = redeemToPublic rsk
        mkWit _ sigData = Identity $ RedeemWitness
            { twRedeemKey = rpk
...
createGenericTx pendingTx creator inputSelectionPolicy utxo outputs addrData =
    runTxCreator inputSelectionPolicy $ do
        ...
        txAux <- (lift . lift) (creator inps outs)
...
createTx pendingTx utxo ss outputs addrData =
    createGenericTxSingle pendingTx (\i o -> return $ makePubKeyTx ss i o)
...
createMOfNTx pendingTx utxo keys outputs addrData =
    createGenericTxSingle pendingTx (\i o -> return $ makeMOfNTx validator sks i o)
    OptimizeForSecurity utxo outputs addrData
  where
    ids = map fst keys
    sks = map snd keys
...
createFakeTxFromRawTx fakeAddr TxRaw{..} =
    ...
    in runIdentity $ makeMPubKeyTxAddrs
           (\_ -> return $ fakeSigner fakeSK)
           trInputs
           txOutsWithRem
